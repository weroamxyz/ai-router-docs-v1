...
| 模型 | 描述 |
| --- | --- |
| **claude-haiku-4-5** | 速度最快且智能水平接近前沿模型 |
| **claude-sonnet-4-5** | Claude Sonnet 4.5 对话模型（上一代） |
| **claude-sonnet-4-6** | Claude Sonnet 4.6 对话模型（上一代） |
| **claude-sonnet-5** | 速度与智能的最佳结合 |
| **claude-opus-4-5** | Claude Opus 4.5 对话模型（上一代） |
| **claude-opus-4-6** | Claude Opus 4.6 对话模型（上一代） |
| **claude-opus-4-7** | Claude Opus 4.7 对话模型（上一代） |
| **claude-opus-4-8** | Claude Opus 4.8 对话模型（上一代） |
| **claude-opus-5** | 适用于复杂的智能体编码和企业级工作 |
| **claude-fable-5** | 面向长时间运行智能体的新一代智能；Anthropic 目前广泛发布的最强大模型 |
