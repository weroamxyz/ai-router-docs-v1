...
| **veo-3.1-generate-preview** | Veo 3.1 文本到视频生成模型 |
| **veo-3.1-fast-generate-preview** | Veo 3.1 快速文本到视频生成模型 |
...
> - `GET /v1/video/generations/{task_id}` — 包装格式 `{"code":"success","data":{...}}`，成功时状态值为 `"succeeded"`。
> - `GET /v1/videos/{task_id}` — 扁平格式 `{"id":"...","status":"completed",...}`，成功时状态值为 `"completed"`。
>
> 两个接口均可用于轮询任务状态，解析时注意不同的 `status` 字段值。
