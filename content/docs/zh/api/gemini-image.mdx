...
```bash
# 基础图片生成
curl "((SITE_URL))/v1beta/models/gemini-2.5-flash-image:generateContent?key=$API_KEY" \
...
```bash
# 图片编辑
curl "((SITE_URL))/v1beta/models/gemini-3-pro-image-preview:generateContent?key=$API_KEY" \
  -H 'Content-Type: application/json' \
  -X POST \
  -d '{
    "contents": [{
