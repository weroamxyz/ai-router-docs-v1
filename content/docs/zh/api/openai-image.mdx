...
<Callout type="info" title="JSON 图片编辑请求">
...
| `size` | 字符串 | 否 | 生成图片的尺寸。标准可选值：`1024x1024`、`1536x1024`（横版）、`1024x1536`（竖版）、`auto`。`gpt-image-2` 和两个 GPT Image 2.5 变体还支持自定义 `宽度x高度` 字符串；请参阅[自定义尺寸](#自定义尺寸)。默认值：`auto`。 |
