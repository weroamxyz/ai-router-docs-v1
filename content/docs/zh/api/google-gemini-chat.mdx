...
| **gemini-3.1-flash-lite** | Gemini 3.1 Flash Lite 对话模型 |
| **gemini-3.1-pro-preview** | Gemini 3.1 Pro 对话模型（预览版） |
...
```bash
curl "https://www.unodetech.xyz/v1beta/models/gemini-3-pro-preview:generateContent?key=$API_KEY" \
...
```bash
curl "https://www.unodetech.xyz/v1beta/models/gemini-3-pro-preview:streamGenerateContent?alt=sse&key=$API_KEY" \
    -H 'Content-Type: application/json' \
    --no-buffer \
    -d '{
      "contents": [{
...
#### 生成内容
```
POST https://www.unodetech.xyz/v1beta/{model=models/*}:generateContent
```

#### 流式生成内容
```
POST https://www.unodetech.xyz/v1beta/{model=models/*}:streamGenerateContent
```
