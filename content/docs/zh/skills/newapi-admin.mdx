...
description: newapi-admin 是 ((BRAND)) 官方管理员级 Skill 插件，支持在 Claude Code、Cursor、Windsurf 等 AI 编辑器中直接执行渠道管理、用户管理、系统配置、日志查询等管理员运维操作。正在开发中，敬请期待。
---
...
**newapi-admin** 是面向 ((BRAND)) 管理员的 Skill 插件。与用户级的 [newapi](/zh/docs/skills/newapi) Skill 不同，newapi-admin 专注于**管理员运维场景**，让站点管理员在 AI 编辑器中通过自然语言指令完成日常管理任务，无需登录管理后台。
