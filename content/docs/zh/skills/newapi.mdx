---
title: newapi
description: newapi 是 ((BRAND)) 官方用户级 Skill 插件，支持 Claude Code、Codex 和 OpenClaw（龙虾）。通过自然语言指令在 AI 编码助手内查询模型、管理令牌、查看分组和余额，并直接提问 ((BRAND)) 的使用问题。
---
...
<Callout type="info" title="什么是 newapi-skills？">
  **newapi-skills** 是 ((BRAND)) 官方推出的 AI 编辑器 Skill 插件。Skill 是一种轻量级扩展协议，让 AI 编码助手（如 Claude Code、Codex、OpenClaw（龙虾）、Cursor、Windsurf、Cline 等）能够在编辑器内**直接调用 ((BRAND)) 接口**，完成模型查询、令牌管理、余额查看，以及提问 ((BRAND)) 部署、配置和 API 调用等使用问题，实现真正的"编码即运维"工作流。

  - GitHub 仓库：[github.com/QuantumNous/skills](https://github.com/QuantumNous/skills)

  现在也支持在 OpenClaw（龙虾）中使用，你可以直接在对话工作流里调用 `/newapi` 指令完成模型查询、令牌管理、余额查看，也能直接提问 ((BRAND)) 的使用问题。
</Callout>

## 为什么使用 Skills

在日常 AI 开发中，开发者经常需要在编辑器和 ((BRAND)) 管理面板之间反复切换：查看可用模型、创建 API 令牌、检查余额、调整分组配额……这些操作打断了编码流程。

**newapi-skills 解决的问题：**

- **零切换**：在 Claude Code、Codex、OpenClaw（龙虾）等 AI 编码助手中直接输入自然语言指令，无需打开浏览器或管理后台
- **内置问答**：可直接提问 ((BRAND)) 的部署、配置、分组、模型调用格式等使用问题，快速获得帮助
...
以下是 newapi-skills 提供的全部指令。这些指令通过 ((BRAND)) REST API 执行，支持查询和管理 ((BRAND)) 实例中的各类资源。

### 查询类指令

| 指令 | 说明 | 用途 |
|------|------|------|
| `/newapi models` | 列出可用 AI 模型 | 查看当前 ((BRAND)) 实例中配置的所有模型（OpenAI、Claude、Gemini 等） |
...
| 指令 | 说明 | 用途 |
|------|------|------|
| `/newapi help <question>` | 提问关于 ((BRAND)) 的问题 | 获取 ((BRAND)) 部署、配置、API 调用格式等帮助信息 |
...
<Steps>
  ...
  <Step>
    ...
    newapi-skills 需要三个环境变量来连接你的 ((BRAND)) 实例。推荐在终端配置文件中设置：
    ...
    | 变量 | 说明 | 示例 |
    |------|------|------|
    | `NEWAPI_BASE_URL` | ((BRAND)) 服务地址 | `https://api.example.com` |
  ...
  <Step>
    ...
    - 输入 `/newapi help 如何配置模型分组和令牌？` 直接提问 ((BRAND)) 的使用问题
...
- GitHub 仓库：[github.com/QuantumNous/skills](https://github.com/QuantumNous/skills)
- ((BRAND)) 官网：[www.newapi.ai](https://www.newapi.ai)
