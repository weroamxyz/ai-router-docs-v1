...
description: AstrBot 配置教程 — 将开源 Agent 聊天机器人平台对接 ((BRAND))，为 QQ、飞书、钉钉、企业微信等即时通讯注入 AI 能力。
---
...
在弹出的对话框中，将 API Base URL 设置为 NewAPI 的接口地址。请使用您自己部署的 ((BRAND))，或确认服务方具备合法上游授权和合规义务的 ((BRAND)) 服务。如果您本地部署了 NewAPI，则填写本地地址，例如 `http://localhost:3000/v1`。不要将来源不明的 API 地址或密钥接入生产环境。
