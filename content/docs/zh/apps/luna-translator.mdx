...
description: LunaTranslator 教程 — 开源 GalGame / 视觉小说翻译器，支持 HOOK、OCR 与 AI 翻译引擎。对接 算力仓 提升翻译质量。
---
...
LunaTranslator 支持接入本地部署的 NewAPI。请使用您自己部署的 算力仓，或确认服务方具备合法上游授权和合规义务的 算力仓 服务。不要将来源不明的 API 地址或密钥接入生产环境。

### 一键配置

1. 在 算力仓 控制台的`系统设置`->`聊天设置`中，添加如下快捷选项：
