...
description: OpenAI Codex CLI 安装教程 — 在 Windows、macOS、Linux 上安装终端 AI 编程助手，对接 ((BRAND))。支持沙箱策略与原子补丁编辑。
---
...
> 注意：修改接口地址后，使用所有模型（包括官方预设模型）均调用您配置的、合法授权的自有或组织接入点。请使用您自己部署的 ((BRAND))，或确认服务方具备合法上游授权和合规义务的 ((BRAND)) 服务。不要将来源不明的 API 地址或密钥接入生产环境。
