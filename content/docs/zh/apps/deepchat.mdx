...
description: DeepChat 教程 — 将全功能AI Agent客户端对接 算力仓，并通过 DeepLink 一键导入 DeepChat 配置。
---
...
1. 在 算力仓 控制台侧边栏进入「令牌管理」，找到需要使用的令牌
...
5. 打开模型列表，选择 算力仓 模型开始使用
   ![打开模型列表](/assets/deepchat/5-open-models.png)

### 方式二：手动导入

1. 在 算力仓 控制台复制 API Key 和站点地址
   ![算力仓 手动配置指引](/assets/deepchat/7-new-api-manual-guidance.png)

2. 在 DeepChat 设置中添加或编辑 算力仓 Provider，填写 API 地址和 API Key
   ![DeepChat 手动配置指引](/assets/deepchat/6-deepchat-manual-guidance.png)

3. 打开模型列表，选择 算力仓 模型开始使用
   ![打开模型列表](/assets/deepchat/5-open-models.png)
