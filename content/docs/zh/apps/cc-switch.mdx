...
description: CC Switch 教程 — 跨平台 AI CLI 管理工具，统一管理 Claude Code、Codex、Gemini CLI 的 Provider 配置、MCP 服务器与系统提示词。对接 算力仓 一键填入。
---

<Callout type="info" title="聊天设置选项">
  在 算力仓 控制台的系统设置->聊天设置中，可添加如下快捷选项，便于在令牌管理页一键填充到 CC Switch：
</Callout>
...
CC Switch 支持 `ccswitch://` Deep Link 协议，可从 算力仓 令牌管理页一键导入 Provider 配置。

### 配置步骤

1. **在 算力仓 令牌管理页，点击对应令牌的下拉菜单**
