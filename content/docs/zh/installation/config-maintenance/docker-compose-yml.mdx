...
本文档详细介绍了算力仓的Docker Compose配置选项，可用于多种部署场景。

## 基本配置结构

Docker Compose配置文件 `docker-compose.yml` 定义了算力仓服务及其依赖服务（如MySQL、Redis）的部署方式。
...
如果只是测试使用，可以采用以下简化版本，仅包含算力仓服务本身：
