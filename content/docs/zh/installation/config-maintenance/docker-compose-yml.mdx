...
本文档详细介绍了((BRAND))的Docker Compose配置选项，可用于多种部署场景。

## 基本配置结构

Docker Compose配置文件 `docker-compose.yml` 定义了((BRAND))服务及其依赖服务（如MySQL、Redis）的部署方式。
...
如果只是测试使用，可以采用以下简化版本，仅包含((BRAND))服务本身：
...
| `environment`    | 环境变量设置，用于配置((BRAND))行为                         |
| `depends_on`     | 依赖服务，确保按正确顺序启动                              |
| `healthcheck`    | 健康检查配置，用于监控服务状态                            |

### 环境变量说明

((BRAND))支持多种环境变量配置，以下是常用的几个：
