...
本文档提供使用 1Panel 面板部署 ((BRAND)) 的图文教程。
...
<Callout type="info" title="搜索应用">
  进入应用商店应用列表，在【AI】分类下找到 ((BRAND)) 应用进行安装
</Callout>
...
<Callout type="info" title="确认安装">
  点击确认开始安装，页面将自动跳转到已安装应用列表，等待 ((BRAND)) 应用状态变为已启动
</Callout>
...
1. 在 1Panel 应用列表中，点击 ((BRAND)) 应用的 **参数** 按钮
2. 在环境变量区域添加：
...
<Callout type="info" title="访问应用">
  - 在 `已安装` 页面，点击 `跳转` 即可进入 ((BRAND)) 的 WebUI 页面
