...
本文档提供了在本地环境中设置和开发 算力仓 项目的详细步骤，适合希望参与项目开发或进行功能定制的开发者。
...
算力仓 支持通过 `.env` 文件配置环境变量。创建 `.env` 文件（可从 `.env.example` 复制）：
...
算力仓 的前端代码位于 `web` 目录中，使用 React 和 [semi design 组件库](https://semi.design/zh-CN) 开发。
