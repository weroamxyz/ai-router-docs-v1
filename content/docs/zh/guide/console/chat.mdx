...
((BRAND))可快速访问一些 AI 聊天应用，可以方便地进行测试或使用
...
<Callout type="info">
Cherry Studio 是一款将多模型能力集成到统一工作空间的高效 AI 工具平台。通过将各类大模型（如 OpenAI、Google、Anthropic、本地模型等）接入到同一个工作界面中，Cherry Studio 让用户可以在一个应用里完成写作、编程、搜索、自动化、知识管理等多种任务。无论你使用的是云端模型还是本地模型，Cherry Studio 都能帮助你以更自然、高效的方式工作。
</Callout>
<Callout type="info">
支持从((BRAND))一键导入配置
</Callout>
