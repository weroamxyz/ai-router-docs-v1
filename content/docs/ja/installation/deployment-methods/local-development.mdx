...
このドキュメントは、Unode プロジェクトをローカル環境でセットアップし開発するための詳細な手順を提供します。プロジェクト開発に参加したい、またはカスタム開発を行いたい開発者向けです。
...
まず、GitHub から Unode リポジトリをローカルにクローンします。
...
Unode は `.env` ファイルを介した環境変数の設定をサポートしています。`.env` ファイルを作成します（`.env.example` からコピーできます）。
...
Unode のフロントエンドコードは `web` ディレクトリにあり、React と [semi design コンポーネントライブラリ](https://semi.design/zh-CN) を使用して開発されています。
