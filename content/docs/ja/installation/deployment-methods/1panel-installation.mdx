...
このドキュメントでは、1Panel パネルを使用して ((BRAND)) をデプロイするための図解チュートリアルを提供します。
...
<Callout type="info" title="アプリケーションの検索">
  アプリケーションストアのアプリケーションリストに入り、【AI】カテゴリで ((BRAND)) アプリケーションを見つけてインストールします。
</Callout>
...
<Callout type="info" title="アプリケーションのインストール">
  インストールページで ((BRAND)) アプリケーションのパラメータを設定します。
</Callout>

- **名前**：作成する ((BRAND)) アプリケーションの名前
- **バージョン**：((BRAND)) のバージョンを選択します
- **ポート**：((BRAND)) アプリケーションのサービスポート
...
<Callout type="info" title="インストールの確認">
  「確認」をクリックしてインストールを開始すると、ページは自動的にインストール済みアプリケーションリストにリダイレクトされ、((BRAND)) アプリケーションの状態が「起動済み」になるまで待ちます。
</Callout>
...
1. 1Panel アプリケーションリストで、((BRAND)) アプリケーションの **パラメータ** ボタンをクリックします
2. 環境変数セクションに追加します：
...
<Callout type="info" title="アプリケーションへのアクセス">
  - 「インストール済み」ページで、「ジャンプ」をクリックすると ((BRAND)) の WebUI ページに入ることができます
...
2. ((BRAND)) アプリケーションを見つけ、**アップグレード** ボタンをクリック
