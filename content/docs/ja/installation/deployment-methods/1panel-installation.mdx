...
このドキュメントでは、1Panel パネルを使用して ComputeVault をデプロイするための図解チュートリアルを提供します。
...
## ステップ2：ComputeVault のインストール

### アプリケーションの検索

<Callout type="info" title="アプリケーションの検索">
  アプリケーションストアのアプリケーションリストに入り、【AI】カテゴリで ComputeVault アプリケーションを見つけてインストールします。
</Callout>
...
<Callout type="info" title="アプリケーションのインストール">
  インストールページで ComputeVault アプリケーションのパラメータを設定します。
</Callout>

- **名前**：作成する ComputeVault アプリケーションの名前
- **バージョン**：ComputeVault のバージョンを選択します
- **ポート**：ComputeVault アプリケーションのサービスポート
...
<Callout type="info" title="インストールの確認">
  「確認」をクリックしてインストールを開始すると、ページは自動的にインストール済みアプリケーションリストにリダイレクトされ、ComputeVault アプリケーションの状態が「起動済み」になるまで待ちます。
</Callout>
...
1. 1Panel アプリケーションリストで、ComputeVault アプリケーションの **パラメータ** ボタンをクリックします
2. 環境変数セクションに追加します：
...
## ステップ4：ComputeVault へのアクセス

<Callout type="info" title="アプリケーションへのアクセス">
  - 「インストール済み」ページで、「ジャンプ」をクリックすると ComputeVault の WebUI ページに入ることができます
...
2. ComputeVault アプリケーションを見つけ、**アップグレード** ボタンをクリック
