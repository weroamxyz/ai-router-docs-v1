...
This document details the Docker Compose configuration options for Unode, applicable to various deployment scenarios.

## Basic Configuration Structure

The Docker Compose configuration file `docker-compose.yml` defines how the Unode service and its dependent services (such as MySQL, Redis) are deployed.
...
For testing purposes, you can use the following simplified version, which only includes the Unode service itself:
...
| `environment`    | Environment variable settings, used to configure Unode behavior              |
...
Unode supports various environment variable configurations. Here are some commonly used ones:
