...
This document details the Docker Compose configuration options for ComputeVault, applicable to various deployment scenarios.

## Basic Configuration Structure

The Docker Compose configuration file `docker-compose.yml` defines how the ComputeVault service and its dependent services (such as MySQL, Redis) are deployed.
...
For testing purposes, you can use the following simplified version, which only includes the ComputeVault service itself:
...
### ComputeVault Service Configuration
...
| `environment`    | Environment variable settings, used to configure ComputeVault behavior              |
...
ComputeVault supports various environment variable configurations. Here are some commonly used ones:
