...
This document provides all environment variables supported by Unode and their configuration instructions. You can customize the system's behavior by setting these environment variables.

<Callout type="info" title="Tip">
  Unode supports reading environment variables from `.env` files. Please refer to the `.env.example`
