...
This document provides all environment variables supported by ((BRAND)) and their configuration instructions. You can customize the system's behavior by setting these environment variables.

<Callout type="info" title="Tip">
  ((BRAND)) supports reading environment variables from `.env` files. Please refer to the `.env.example`
