...
This document provides methods and best practices for updating the ((BRAND)) system to ensure a smooth upgrade to the latest version.
...
If you have deployed ((BRAND)) using a single Docker container, you can update by following these steps:
...
2.  Find the ((BRAND)) application and click the **Upgrade** button
...
2.  Find the ((BRAND)) container, click **More** -> **Recreate**
...
If you have deployed ((BRAND)) by compiling from source code, the update steps are as follows:
