...
This document provides a graphical tutorial for deploying Unode using the 1Panel panel.
...
## Step 2: Install Unode

### Search for Applications

<Callout type="info" title="Search for Applications">
  Go to the application list in the App Store, find the Unode application under the [AI] category, and install it.
</Callout>
...
<Callout type="info" title="Install Application">
  Configure Unode application parameters on the installation page:
</Callout>

- **Name**: The name of the Unode application to be created
- **Version**: Select the version of Unode
- **Port**: The service port for the Unode application
...
<Callout type="info" title="Confirm Installation">
  Click Confirm to start the installation. The page will automatically redirect to the installed applications list. Wait for the Unode application status to change to Started.
</Callout>
...
1. In the 1Panel application list, click the **Parameters** button for the Unode application
...
<Callout type="info" title="Access Application">
  - On the `Installed` page, click `Jump` to enter the Unode WebUI page
...
2. Find the Unode application, click the **Upgrade** button
