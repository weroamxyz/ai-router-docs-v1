...
| Model | Description |
| --- | --- |
| **gemini-2.5-flash** | Best price-performance model for low-latency, high-volume tasks that require reasoning |
| **gemini-2.5-pro** | Most advanced 2.5-family model for complex tasks, with deep reasoning and coding capabilities |
| **gemini-3-flash-preview** | Frontier-class performance rivaling larger models at a fraction of the cost (preview) |
| **gemini-3.1-flash-lite** | Frontier-class performance rivaling larger models at a fraction of the cost |
| **gemini-3.1-pro-preview** | Advanced intelligence, complex problem-solving, and powerful agentic and vibe coding capabilities (preview) |
| **gemini-3.5-flash-lite** | Fastest, most cost-effective 3.5 model, built for high-throughput execution |
| **gemini-3.5-flash** | Legacy Flash model providing baseline speed and foundational performance for routine, high-throughput workloads |
| **gemini-3.6-flash** | Previous-generation Flash model balancing speed and multimodal capabilities across general agentic and everyday tasks |
| **gemini-3.7-flash** | Latest and most capable Flash model, built for complex coding, agentic workflows, and reliable multi-step execution |
...
```bash
curl "((SITE_URL))/v1beta/models/gemini-3-pro-preview:generateContent?key=$API_KEY" \
...
```bash
curl "((SITE_URL))/v1beta/models/gemini-3-pro-preview:streamGenerateContent?alt=sse&key=$API_KEY" \
    -H 'Content-Type: application/json' \
    --no-buffer \
    -d '{
      "contents": [{
...
#### Generate Content
```
POST ((SITE_URL))/v1beta/{model=models/*}:generateContent
```

#### Stream Generate Content
```
POST ((SITE_URL))/v1beta/{model=models/*}:streamGenerateContent
```
