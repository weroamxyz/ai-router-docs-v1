...
| Model | Description |
| --- | --- |
| **grok-imagine-video** | Grok Imagine video generation model (text-to-video, image-to-video, reference-to-video, editing, extension) |
...
1. Submit Task: Send a text prompt (and optionally one or more images, or a source video) to create a video task
...
The same `/v1/video/generations` endpoint serves five use cases, selected by the request body: text-to-video, image-to-video, and reference-to-video (all via `metadata.mode` `generate`, the default), plus video editing (`metadata.mode` `edit`) and video extension (`metadata.mode` `extend`).
...
Edit an existing video by describing the change in `prompt`. Set `metadata.mode` to `"edit"` and pass the source video URL (or base64 data URI) in `metadata.video_url`. The output inherits the source video's duration, resolution, and aspect ratio (capped at `720p`), so `duration`, `size`, and `aspect_ratio` are ignored in this mode.
...
| model | string | Yes | Model name: `grok-imagine-video` |
| metadata | object | Yes | Must include `mode` and `video_url` for this mode |
| metadata.mode | string | Yes (this mode) | Set to `edit` |
| metadata.video_url | string | Yes (this mode) | Source video URL or base64 data URI |

> **Note:** `duration`, `size`, and `metadata.aspect_ratio` are ignored for editing — the output matches the source video (capped at `720p`).

### 5. Video Extension

Extend an existing video with new content described in `prompt`. Set `metadata.mode` to `"extend"` and pass the source video URL (or base64 data URI) in `metadata.video_url`. Here `duration` is the length of the **appended segment only**, not the total output — e.g. a 10-second source with `duration` `5` yields a 15-second result.
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| prompt | string | Yes | Text prompt describing the extension |
| model | string | Yes | Model name: `grok-imagine-video` |
| duration | integer | Yes | Length of the appended segment only, in seconds. Allowed: integers from `1` to `15`, inclusive |
| metadata | object | Yes | Must include `mode` and `video_url` for this mode |
| metadata.mode | string | Yes (this mode) | Set to `extend` |
| metadata.video_url | string | Yes (this mode) | Source video URL or base64 data URI |
...
| data.action | string | `textGenerate` (text-to-video), `generate` (image-to-video), `referenceGenerate` (reference-to-video), `edit` (editing), or `extend` (extension) |
