...
| **veo-3.1-generate-preview** | Veo 3.1 text-to-video generation model |
| **veo-3.1-fast-generate-preview** | Veo 3.1 fast text-to-video generation model |
...
```bash
curl -X POST "https://www.unodetech.xyz/v1/video/generations" -H "Content-Type: application/json" -H "Authorization: Bearer API_KEY" -d @veoTest.json
```
...
```bash
curl -X GET "https://www.unodetech.xyz/v1/video/generations/TASK_ID" -H "Authorization: Bearer API_KEY"
```
...
```json
{
  ...
  "data": {
    ...
    "status": "succeeded",
    "task_id": "TASK_ID",
    "url": "https://www.unodetech.xyz/v1/videos/TASK_ID/content"
  }
}
```
