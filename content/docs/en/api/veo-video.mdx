...
```bash
curl -X POST "((SITE_URL))/v1/video/generations" -H "Content-Type: application/json" -H "Authorization: Bearer API_KEY" -d @veoTest.json
```
...
```bash
curl -X GET "((SITE_URL))/v1/video/generations/TASK_ID" -H "Authorization: Bearer API_KEY"
```
...
```json
{
  ...
  "data": {
    ...
    "status": "succeeded",
    "task_id": "TASK_ID",
    "url": "((SITE_URL))/v1/videos/TASK_ID/content"
  }
}
```
