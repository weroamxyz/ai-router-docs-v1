...
| --- | --- | --- |
| **veo-3.0-generate-001** | Veo 3.0 text-to-video generation model | ✅ Available |
| **veo-3.0-fast-generate-001** | Veo 3.0 fast text-to-video generation model | ✅ Available |
| **veo-3.1-generate-001** | Veo 3.1 text-to-video generation model | ⚠️ Temporarily Unavailable |
| **veo-3.1-fast-generate-001** | Veo 3.1 fast text-to-video generation model | ⚠️ Temporarily Unavailable |
...
```bash
curl -X POST "https://unodetech.xyz/v1/video/generations" -H "Content-Type: application/json" -H "Authorization: Bearer API_KEY" -d @veoTest.json
```
