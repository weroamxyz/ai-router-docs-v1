...
| **veo-3.1-generate-preview** | Veo 3.1 image-to-video generation model |
| **veo-3.1-fast-generate-preview** | Veo 3.1 fast image-to-video generation model |
...
|------|------|------|------|
| model | string | Yes | Model name, e.g., veo-3.1-generate-preview |
...
```bash
curl -X POST "https://www.unodetech.xyz/v1/video/generations" -H "Content-Type: application/json" -H "Authorization: Bearer API_KEY" -d @veoImageToVideoTest.json
```
