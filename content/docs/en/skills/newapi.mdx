---
title: newapi
description: newapi is the official ((BRAND)) user-level Skill for Claude Code, Codex, and OpenClaw. Query models, manage tokens, view groups, check balance, and ask ((BRAND)) usage questions directly inside your AI coding workflow.
---
...
<Callout type="info" title="What is newapi-skills?">
  **newapi-skills** is the official AI editor Skill plugin from ((BRAND)). Skills are a lightweight extension protocol that enables AI coding assistants — such as Claude Code, Codex, OpenClaw, Cursor, Windsurf, and Cline — to **call ((BRAND)) endpoints directly from within the editor**, performing model queries, token management, balance checks, and answering ((BRAND)) deployment, configuration, and API usage questions without ever leaving your coding environment.

  - GitHub Repository: [github.com/QuantumNous/skills](https://github.com/QuantumNous/skills)

  It now also works inside OpenClaw, so you can run `/newapi` commands for models, tokens, groups, and balance, and ask ((BRAND)) usage questions without leaving your agent workflow.
</Callout>

## Why Use Skills

During everyday AI development, developers frequently switch between their editor and the ((BRAND)) management dashboard — checking available models, creating API tokens, inspecting balance, adjusting group quotas. These context switches disrupt coding flow.
...
- **Built-in help**: Ask about ((BRAND)) deployment, configuration, groups, and API call formats directly from your coding workflow
...
Below is the complete list of commands provided by newapi-skills. These commands interact with the ((BRAND)) REST API to query and manage resources on your ((BRAND)) instance.

### Query Commands

| Command | Description | Use Case |
|---------|-------------|----------|
| `/newapi models` | List available AI models | View all models configured on your ((BRAND)) instance (OpenAI, Claude, Gemini, etc.) |
...
| Command | Description | Use Case |
|---------|-------------|----------|
| `/newapi help <question>` | Ask about ((BRAND)) | Get help on ((BRAND)) deployment, configuration, API call formats, and more |
...
<Steps>
  ...
  <Step>
    ...
    newapi-skills requires three environment variables to connect to your ((BRAND)) instance. Add them to your shell profile:
    ...
    |----------|-------------|---------|
    | `NEWAPI_BASE_URL` | ((BRAND)) service URL | `https://api.example.com` |
  ...
  <Step>
    ...
    - Type `/newapi help how do I configure groups and tokens?` to ask ((BRAND)) usage questions directly
...
- ((BRAND)) GitHub: [github.com/QuantumNous/new-api](https://github.com/QuantumNous/new-api)
