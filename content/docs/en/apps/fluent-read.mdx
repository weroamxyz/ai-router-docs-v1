...
description: FluentRead configuration guide — open-source browser translation plugin with 20+ engines and AI model support. Connect to ((BRAND)) for immersive bilingual reading.
---

<Callout type="info" title="Chat Settings Options">
  In the System Settings -> Chat Settings of the ((BRAND)) console, you can add
...
### Import Configuration from ((BRAND)) Console (Recommended)

After the FluentRead plugin is installed in your browser, opening the ((BRAND)) console -> Token Management page will display a prompt to add FluentRead.
...
After confirming the import, the ((BRAND)) configuration in FluentRead will be enabled.
