...
description: LunaTranslator Tutorial — An open-source GalGame / visual novel translator that supports HOOK, OCR, and AI translation engines. Integrates with ((BRAND)) to improve translation quality.
---
...
LunaTranslator supports integration with locally deployed ((BRAND)). Please use your own deployed ((BRAND)), or ensure that the service provider has legitimate upstream authorization and compliance obligations for their ((BRAND)) service. Do not connect API addresses or keys from unknown sources to production environments.

### One-Click Configuration

1.  In the ((BRAND)) console's `System Settings` -> `Chat Settings`, add the following shortcut option:
