...
description: LunaTranslator Tutorial — An open-source GalGame / visual novel translator that supports HOOK, OCR, and AI translation engines. Integrates with Unode to improve translation quality.
---
...
LunaTranslator supports integration with locally deployed Unode. Please use your own deployed Unode, or ensure that the service provider has legitimate upstream authorization and compliance obligations for their Unode service. Do not connect API addresses or keys from unknown sources to production environments.

### One-Click Configuration

1.  In the Unode console's `System Settings` -> `Chat Settings`, add the following shortcut option:
