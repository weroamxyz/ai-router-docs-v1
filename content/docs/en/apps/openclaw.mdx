...
description: OpenClaw Tutorial — Install OpenClaw, integrate ((BRAND)), and quickly set up a self-hosted AI assistant. Open-source project, supporting multi-channel integration like Telegram, Discord, WhatsApp.
---
...
OpenClaw is completely open-source. You can browse the source code, submit issues, or contribute on [OpenClaw's GitHub repository](https://github.com/openclaw/openclaw). This tutorial covers the complete steps for installing, configuring, and integrating OpenClaw with ((BRAND)).
...
<Callout type="info" title="Preparation Information">
  ...
  - An available ((BRAND)) address (usually ending with `/v1`)
  - An available ((BRAND)) API Key
  - Please use your own deployed ((BRAND)), or confirm that the service provider has legitimate upstream authorization and compliance obligations for their ((BRAND)) service. Do not connect unknown API addresses or keys to production environments.
</Callout>

Before integrating ((BRAND)), it's recommended to first get the Gateway and Control UI running according to OpenClaw's official current recommended process. This makes it easier to distinguish whether OpenClaw itself is not starting or if the model provider configuration is incorrect when troubleshooting later.
...
This wizard will complete basic authentication, Gateway setup, and optional channel initialization. The goal here is to get OpenClaw running first, then switch the default model to ((BRAND)) later.
...
## 🚀 Using ((BRAND)) as a Model Provider

OpenClaw supports integrating custom or OpenAI-compatible model gateways via `models.providers`. For ((BRAND)), the most common approach is to add it as a custom provider to the configuration, then point the default model to `newapi/model-ID`.

### Integration Approach

1. Declare a `newapi` provider under `models.providers`.
2. Point `baseUrl` to your ((BRAND)) address, ensuring it includes `/v1`.
...
First, provide your ((BRAND)) key in the current shell, service environment, or an `.env` file readable by OpenClaw:
...
This is not a complete configuration that must be copied exactly, but rather the most critical part for integrating ((BRAND)). As long as the `provider`, model IDs, and default model references are correct, OpenClaw will be able to call your exposed model resources via ((BRAND)).
...
| `models.providers.newapi.baseUrl` | Your ((BRAND)) address, usually needs to include `/v1` |
| `models.providers.newapi.apiKey` | ((BRAND)) key, recommended to inject via `${NEWAPI_API_KEY}` |
| `models.providers.newapi.api` | For OpenAI-compatible gateways like ((BRAND)), use `openai-completions` |
| `models.providers.newapi.models` | The model IDs listed here must match the actual model names exposed by your ((BRAND)) |
