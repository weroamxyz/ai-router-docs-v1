...
In the ((BRAND)) system, ratios are key parameters for calculating quota consumption. Quota is the internal billing unit of the system, and all API calls are ultimately converted into quota points for deduction.
...
#### Audio Models (Special handling, automatically processed internally by ((BRAND)))
...
((BRAND)) adopts a dual billing mechanism of pre-consumption and post-consumption:
